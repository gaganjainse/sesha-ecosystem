...
banned reintroducing "Shesha". In practice, 8 component packages plus desktop-layer
...
|---|---|---|---|
| shesh-audit | dist/import `shesha_audit`→`shesh_audit`; data dir migration | 20/20 | 7bab045 |
| shesh-backup | + `~/.local/state/shesha→shesh` migration | 8/8 | b5e00ee |
| shesh-files | + env `SHESHA_*`→`SHESH_*` | 5/5 | 1bf458f |
...
| shesh-desktop | tools/, sdata/, profiles/, QML namespace `options.shesha→shesh`, wake word "Hey Shesh", docs/ | 26/26 | …ea1f179 |
...
- ❌ Local checkouts/installs from before 2026-08-12 must re-clone or run the
  rename tooling; entry-point names changed (`shesha-audit-mcp`→`shesh-audit-mcp`).
