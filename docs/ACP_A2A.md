...
Two protocols Sesha adopts alongside MCP so it can live in editors and coordinate agents.
...
The ACP server spawns `sesha-orchestrator` (coder role) as its agent, handing it the MCP
...
- **Why:** lets Sesha's specialist subagents talk directly (coordinator→researcher→critic) and,
  later, lets remote agents participate without us inventing a protocol.
- **Component:** `sesha-orchestrator` speaks A2A on a local Unix socket (P1).
...
Editor (Zed/JetBrains)
   ...
   ▼
sesha-acp ──► sesha-orchestrator (coordinator)
                   ...
                   │                       └─ critic ────┘
                   ▼
              Brain (NexusAOS policy + audit event log)
                   │ MCP (stdio JSON-RPC)
                   ▼
   sesha-files / sesha-shell / sesha-system / sesha-skills / ...
```
