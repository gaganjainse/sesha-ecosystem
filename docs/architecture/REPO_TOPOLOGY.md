...
① UPSTREAM FORKS          (mirrors of external projects, track main/default)
        ...
        ▼
② COMPONENT REPOS        (one per Shesha organ; our code + vendored fork pins)
        │  tagged releases, semver, individually tested
        ▼
③ ECOSYSTEM INTEGRATION  (shesha-ecosystem: manifests pin component versions)
        │  built together, full integration tests → "canary"
        ▼
④ CANARY REPO            (shesha-ecosystem:canary branch — daily build, bleeding edge)
        │  soak on a spare machine/VM for N days; gates pass
        ▼
⑤ STABLE / DOTFILES      (shesha-desktop:main — your actual machine, production)
```

### ① Upstream forks (`gaganjainse/fork-<project>`)
- A bare fork of each external project we use, with a tiny `shesha/` branch carrying our patches
...
### ② Component repos (`shesha-<organ>`)
- One repository per body organ, e.g.:
  - `shesha-brain` — packaging/patches around SheshaAOS kernel for desktop.
  - `shesha-mind` — model routing + specialist prompts (from SheshaOS).
  - `shesha-voice` — Newelle fork/config + STT/TTS + wake word.
  - `shesha-files` — smart-organizer v2 (Rust watcher + Python classifier).
  - `shesha-shell` — Hyprland/MCP control (hyprland-control MCP).
  - `shesha-system` — power/GPU/MUX/backup/maintenance MCP.
  - `shesha-memory` — rag-service wrapper (sensory/long-term memory).
  - `shesha-phone` — ADB Android harness.
  - `shesha-audit` — the append-only event log/policy (SheshaAOS bridge).
- Each has its own tests, its own semver tag, and a `manifest.toml` declaring dependencies.
- Components are independently usable (you can run `shesha-files` without the voice).

### ③ Ecosystem integration (this repo, `shesha-ecosystem`)
...
- Produces a lockfile (`shesha.lock`) like a distro repo snapshot.
...
### ⑤ Stable → your machine (`shesha-desktop:main`)
...
|---|---|
| ① Fork | upstream tests pass; our `shesha/` branch rebases cleanly; license check |
...
```toml
[component.shesha-voice]
repo    = "gaganjainse/shesha-voice"
...
tag SHAs, and writes `shesha.lock`. This is the "package repo" metadata, in one auditable file.
...
- **We start narrow.** Phase 1 builds 3 components (files, shell, system) that already exist in
  shesha-desktop; the rest are added as the gates and time allow.

---

## 5. Repository creation order

1. `shesha-ecosystem` (this repo — the orchestrator/manifests).
2. Forks of the first-wave upstreams (Newelle, end-4/dots-hyprland, plus the Rust/Python deps).
3. Component repos split out from the working shesha-desktop code:
   `shesha-files`, `shesha-shell`, `shesha-system`, `shesha-voice`.
4. `shesha-audit` (brain bridge) once MCP is stable.
