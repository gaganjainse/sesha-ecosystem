...
|---|---|---|
| **Rust** | Brain/kernel, event store, policy engine, file watcher, low-level daemons, eBPF (Aya), security-critical tools | Memory safety without GC, single static binary, `cargo`/`clippy`, your existing NexusAOS/nexus-kernel code |
...
|---|---|---|
| **Zig** | ❌ Don't add | Excellent language, but it would duplicate Rust in the "systems" slot and add a second toolchain + FFI (you already replaced a Zig VT100 FFI with Rust `vte` in nexus-kernel ADR-002). Only use if *forced* by a dependency (none currently). |
...
1. **Process boundaries, not linkage.** Every component is a separate process speaking **MCP (stdio) or
   JSON-RPC (Unix socket)** — exactly how NexusAOS (`nexusaos-rpc`) and Newelle already work. There is
