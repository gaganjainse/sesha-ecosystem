...
> What turns Sesha from a demo into a full ecosystem. We **build** the Sesha-specific organs
...
|---|---|---|
| sesha-system | gaganjainse/sesha-system | power/GPU/MUX/backup, system status |
| sesha-shell | gaganjainse/sesha-shell | Hyprland/Quickshell control |
| sesha-files | gaganjainse/sesha-files | real-time organizer (Rust+Python) |
| sesha-skills | gaganjainse/sesha-skills | notes, web, git, docs, reminders + skill library |
| sesha-voice | (fork of Newelle) | STT/TTS/wake word, chat UI |
| sesha-audit/brain/mind/memory/phone | planned | governance, routing, RAG, ADB |

## 2. Mature MCP servers we package (do NOT rebuild)

Pin each in a component repo; run as a stdio MCP server behind the Sesha policy.
...
| Search (optional) | **Tavily MCP** / Brave | varies | API-key search; DuckDuckGo in sesha-skills needs no key |
...
## 3. Skills library (Markdown, in sesha-skills/skills)
...
- **terminal-ops** — safe shell patterns over SSH (wraps nexusaos-terminal patterns).
...
2. sets its allowed directories/env in a config under `~/.config/sesha/mcp/`,
3. is registered in `manifests/components.toml`,
4. passes the same lint/test/license gate,
5. runs through `sesha-audit` so every tool call is logged.
